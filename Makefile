...
export CURRENT_VERSION:=$(shell cat ${ROOTDIR}/drltf/_meta.py  | sed -En "s/^__version__\s*=\s*[ubrf]*['\"]([^'\"]+)['\"].*$$/\1/p")
...
uninstall: ## uninstall the package
	pip uninstall -y $(PIP_ARGS) drltf || true
...
lint: ## static code analysis with pylint
	pylint --rcfile drltf/.pylintrc -j 0 drltf
...
code-locs: ## display metrics (LOCs, number of comments, etc.)
	radon raw --summary drltf

code-complexity: ## check cyclomatic complexity using radon
	xenon --max-absolute B --max-modules A --max-average A drltf
...
security: ## check source code for vulnerabilities
	@[ "${REPORT_FORMAT}" ] && ( mkdir -p docs/_build/security && bandit -v -r -f ${REPORT_FORMAT} -o docs/_build/security/index.html drltf &> /dev/null ) || true
	bandit -v -r drltf
